"""
Config for the XLeVR teleoperation system.
Loads from config.yaml if it exists, falls back to hardcoded defaults.
"""

import os
try:
    import yaml  # optional: config.yaml overrides; hardcoded DEFAULT_CONFIG used if absent
except ImportError:
    yaml = None
...
def load_config(config_path: str = "config.yaml") -> dict:
    ...
    if yaml is None:
        # pyyaml not installed — the hardcoded DEFAULT_CONFIG already matches the
        # vendored config.yaml, so this is fine; no warning needed.
        return config
...
# Vendored config.yaml lives one level up from this package (station/vr/config.yaml),
# so resolve it deterministically regardless of the process cwd.
_VENDORED_CONFIG = os.path.join(os.path.dirname(os.path.dirname(__file__)), "config.yaml")
_config_data = load_config(_VENDORED_CONFIG)
...
@dataclass
class XLeVRConfig:
    """Main config — only VR + HTTPS are active; keyboard is disabled."""
